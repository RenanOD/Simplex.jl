...
function test_simplexlu()
  ...
  @testset "Optimal solution without sufficient condition" begin
   ...
   sol = [0; 0]
   basis = [2]
   x, z, status = simplexlu(c, A, b, basis)
   @test x ≈ sol
   @test z ≈ dot(sol, c)
   @test status == :Optimal
  end
  ...
  @testset "Transport tests" begin
    @testset "scale $scale_m × $scale_n" for scale_m = 10.^(0:1), scale_n = 10.^(0:1)
      for t = 1:10
        ...
        x, z, status = simplexlu(c, A, b, max_iter = 1000 * scale_m * scale_n)
        Δz = dot(c, x) - zj
        # @test x ≈ xj
